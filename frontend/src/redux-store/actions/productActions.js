...
export const deleteProduct = (id) => async (dispatch, getState) => {
  try {
    ...
    dispatch({
      type: PRODUCT_CONSTANT_TYPES.PRODUCT_DELETE_FAIL,
      payload:
        error.response && error.response.data.message ? error.response.data.message : error.message,
    });
  }
...
export const createProduct = () => async (dispatch, getState) => {
  try {
    ...
    dispatch({
      type: PRODUCT_CONSTANT_TYPES.PRODUCT_CREATE_FAIL,
      payload:
        error.response && error.response.data.message ? error.response.data.message : error.message,
    });
  }
...
export const updateProduct = (product) => async (dispatch, getState) => {
  try {
    ...
    dispatch({
      type: PRODUCT_CONSTANT_TYPES.PRODUCT_UPDATE_FAIL,
      payload:
        error.response && error.response.data.message ? error.response.data.message : error.message,
    });
  }
...
export const createProductReview = (productId, review) => async (dispatch, getState) => {
  try {
    ...
    dispatch({
      type: PRODUCT_CONSTANT_TYPES.PRODUCT_CREATE_REVIEW_FAIL,
      payload:
        error.response && error.response.data.message ? error.response.data.message : error.message,
    });
  }
...
export const listTopProducts = () => async (dispatch) => {
  try {
    dispatch({ type: PRODUCT_CONSTANT_TYPES.PRODUCT_TOP_REVIEWED_REQUEST });

    const { data } = await axios.get("/api/products/top");

    dispatch({
      type: PRODUCT_CONSTANT_TYPES.PRODUCT_TOP_REVIEWED_SUCCESS,
      payload: data,
    });
  } catch (error) {
    dispatch({
      type: PRODUCT_CONSTANT_TYPES.PRODUCT_TOP_REVIEWED_FAIL,
      payload:
        error.response && error.response.data.message ? error.response.data.message : error.message,
    });
  }
